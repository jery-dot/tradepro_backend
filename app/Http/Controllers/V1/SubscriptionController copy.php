...
class SubscriptionController extends Controller
{
    ...
    public function prepareSubscription1(Request $request)
    {
        try {
    ...
    // 1. Attach Payment Method to Customer$paymentMethod = \Stripe\PaymentMethod::retrieve($request->payment_method_id);
    $paymentMethod->attach(['customer' => $user->stripe_id]);

    // 2. Set as Default    \Stripe\Customer::update($user->stripe_id, [
        'invoice_settings' => ['default_payment_method' => $paymentMethod->id],
    ]);
    // 3. Create Subscription (status will be 'incomplete')$subscription = \Stripe\Subscription::create([
        'customer' => $user->stripe_id,
        'items' => [['price_data' => [
    ...
    // 4. Save to your local DB$userSub = UserSubscription::create([
        'user_id' => $user->id,
