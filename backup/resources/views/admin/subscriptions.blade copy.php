        ...
        <table>
            ...
            <tbody>
                @forelse($subscriptions as $sub)
                    @php
                        $typeBadge = ['contractor'=>'bn','subcontractor'=>'bo','labour'=>'bg','apprentice'=>'bp'][$sub->user->type] ?? 'bgr';
                        $typeLabel = ['contractor'=>'Contractor','subcontractor'=>'Sub-contractor','labour'=>'Labour','apprentice'=>'Apprentice'][$sub->user->type] ?? $sub->user->type;
                        $planBadge = ['Contractor'=>'bo','Labourer'=>'bn','Apprentice'=>'bp'][$sub->plan_name] ?? 'bgr';
                        $statusBadge = ['active'=>'bg','cancelled'=>'br','trial'=>'ba','expired'=>'bgr'][$sub->status] ?? 'bgr';
                    @endphp
                    <tr>
                            ...
                            <div style="display:flex;gap:4px">
                                ...
                                @if($sub->status === 'active')
                                    <form method="POST"
                                          ...
                                          onsubmit="return confirm('Cancel subscription for {{ $sub->user->first_name }}?')">
                                        @csrf @method('PATCH')
